...
function FavoritesList({ favoriteCities, removeFromFavorites }) {
    ...
    <>
      {favoriteCities.length === 0 ? (
        <div className="flex flex-col justify-center items-center bg-white/20 backdrop-blur-md w-[780px] rounded-r-md">
          <img className="h-74 mt-4" src="https://www.svgrepo.com/show/489659/empty-box.svg" alt="" />
          <p className="text-white font-bold text-2xl p-10">Favorites is empty!</p>
        </div>
      ) : (
            ...
            >
              <div className="p-4 rounded-lg h-full flex flex-col justify-between text-2xl">
                <div className="flex items-center gap-1">
                  ...
                  <img 
                    onClick={() => handleRemoveClick(city.name)} 
                    className="h-8 cursor-pointer" 
                    src="https://www.svgrepo.com/show/236615/remove-delete.svg" 
                    alt="Remove" 
                  />
                </div>
              ...
              <div className="w-[250px] flex justify-center">
                {city.weather?.[0]?.main === "Clear" && (
                  <img src="https://www.svgrepo.com/show/398422/sun-with-face.svg" alt="Clear" />
                )}
                {city.weather?.[0]?.main === "Clouds" && (
                  <img src="https://www.svgrepo.com/show/276635/cloudy-cloud.svg" alt="Clouds" />
                )}
                {city.weather?.[0]?.main === "Rain" && (
                  <img src="https://www.svgrepo.com/show/313157/rainy.svg" alt="Rain" />
                )}
                {city.weather?.[0]?.main === "Haze" && (
                  <img src="https://www.svgrepo.com/show/396511/foggy.svg" alt="Haze" />
                )}
                {city.weather?.[0]?.main === "Mist" && (
                  <img src="https://www.svgrepo.com/show/474591/fog.svg" alt="Mist" />
                )}
              </div>
            </div>
          ))}
